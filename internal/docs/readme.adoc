...
- name "{{ join \\"col\\" field.name }}" - defines constant names as 'col' appended by the associated field name.
...
- list . - generates the `columns` method that returns constant values. It can be used to build sql queries like INSERT, SELECT.
